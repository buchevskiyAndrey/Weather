...
class WeatherViewModel {
    ...
    func returnTempUnit() -> String {
        var result = ""
        if weather.value?.metric == TemperatureUnit.celsius.rawValue {
            result = "°С"
        } else if weather.value?.metric == TemperatureUnit.fahrenheit.rawValue {
            result = "°F"
        }
        return result
    }
